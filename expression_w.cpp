...
vector<string> ToPostfix(const vector<string>& tokens) {
	...
	for (const auto& token : tokens) {  // auto: const string&
...
shared_ptr<TreeNode> BuildTreeFromPostfix(const vector<string>& postfix) {
	...
	for (const auto& token : postfix) {  // auto: const string&
		if (IsNumber(token) || IsVariable(token)) {
			...
			auto right = st.top();  // auto: shared_ptr<TreeNode>
			st.pop();
			auto left = st.top();   // auto: shared_ptr<TreeNode>
			st.pop();

			auto node = make_shared<TreeNode>(token);  // auto: shared_ptr<TreeNode>
			node->left = left;
			node->right = right;
			st.push(node);
		}
	...
	if (st.size() != 1) {
		throw runtime_error("Tree construction error.");
	}
...
int ExpressionTree::EvaluateNode(shared_ptr<TreeNode> node, int x) {
	if (!node) {
		throw runtime_error("Empty tree node.");
	}
...
void ExpressionTree::NormalizeNode(shared_ptr<TreeNode>& node) {
	if (!node || !IsOperator(node->value[0])) {
		return;
	}
...
void WriteResultToFile(const string& filename, int result, ExpressionTree& tree) {
	...
	out << "Infix form with parentheses: ";
